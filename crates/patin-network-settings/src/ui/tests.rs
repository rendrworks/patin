...
fn scan_row_keeps_network_buttons_inside_minimum_height() {
    ...
    assert!(
        ui.buttons
            .iter()
            .all(|button| { button.bounds.origin.y + button.bounds.size.height <= ui.size.height })
    );
}
...
fn available_network_rows_use_icons_without_numeric_percentages() {
    ...
    assert!(
        forget.bounds.origin.x + forget.bounds.size.width <= ui.size.width - 16.0 + f32::EPSILON
    );
}
