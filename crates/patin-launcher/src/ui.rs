...
impl Launcher {
    ...
    fn layout(&mut self) {
        let content_height = (self.size.height - VERTICAL_PADDING * 2.0).max(ROW_HEIGHT);
        ...
        self.rows = (self.first_visible..end)
            ...
            .map(|(slot, application)| ListRow {
                bounds: Rect::new(
                    HORIZONTAL_PADDING,
                    VERTICAL_PADDING + slot as f32 * ROW_HEIGHT,
                    (self.size.width - HORIZONTAL_PADDING * 2.0).max(1.0),
                    ROW_HEIGHT,
                ),
                application,
            })
        ...
        if self.applications.is_empty() {
            commands.push(text(
                Rect::new(
                    HORIZONTAL_PADDING,
                    VERTICAL_PADDING,
                    self.size.width - HORIZONTAL_PADDING * 2.0,
                    ROW_HEIGHT,
                ),
...
mod tests {
    ...
    fn fixed_window_lists_only_rows_that_fit() {
        ...
        assert!(launcher.rows.iter().all(|row| {
            row.bounds.origin.y >= 0.0 && row.bounds.origin.y + row.bounds.size.height <= 350.0
        }));
    }
    ...
    fn scrolling_moves_the_visible_window_and_clamps_at_each_end() {
        ...
        assert_eq!(launcher.first_visible, 12);
